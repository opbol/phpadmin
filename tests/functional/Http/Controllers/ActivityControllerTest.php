...
class ActivityControllerTest extends FunctionalTestCase {
	/**
	 * @var \App\Services\Logging\UserActivity\Logger
	 */
	public $logger;

	public function setUp() {
		parent::setUp();
		$this->logger = app(App\Services\Logging\UserActivity\Logger::class);
	...
	public function test_display_activities_for_user() {
		...
		$this->visit('dashboard/activity')
			...
			->seePageIs("dashboard/activity/user/{$user->id}/log")
	...
	public function test_search_activities() {
		...
		$this->seePageIs('dashboard/activity?search=fo');
